...
1.  **数据预处理**：compress.py 读取原始 TXT，按号码/UID 前3位分片，存入 data_gzip/xxx.gz。
...
1.  修改 compress.py 中的 `SOURCE_FOLDER` 为你的 TXT 文件路径。
2.  运行脚本：
    ```bash
    python compress.py
    ```
3.  等待完成，你将获得一个 `data_gzip` 文件夹。
...
本项目支持 **GitHub Action 自动化构建**，你可以直接下载构建好的离线镜像包，无需在 NAS 上编译。

### 方法 A：使用 Release 离线包 (推荐)

1.  **下载镜像**：
    前往本仓库的 [Releases](../../releases) 页面，下载对应版本的镜像包 (例如 `nas-search-2601.tar.gz`)。
2.  **导入 NAS**：
    - 打开 **Container Station** -> **镜像 (Images)** -> **导入 (Import)**。
    - 选择下载的 `.tar.gz` 文件。
3.  **上传数据**：
    将第一步生成的 `data_gzip` 文件夹上传到 NAS (例如 `/share/Container/SearchData/data_gzip`)。
4.  **创建容器**：
    - 镜像选择：`nas-search:2601`
    - **挂载卷 (Volumes) [关键]**：
      - 主机路径：`/share/Container/SearchData/data_gzip` (你的数据路径)
      - **容器路径**：`/data` (必须是这个)
    - 端口映射：`8090:8000`
